...
class OIDCAuthenticator {
  constructor(config) {
    this.config = {
      ...
      backendSecret: config.backendSecret || null,  // Auth secret for backend
  ...
  /**
   * Log only to file (not to console) - for request logging in daemon mode
   * This ensures daemon stays quiet on stdout after startup
   */
  logToFileOnly(message) {
    // Only write to log file, never to console
    this.writeToLogFile(message);
  }
  ...
  async startDaemon() {
    ...
    if (preConfiguredTokens) {
      ...
      this.log('   Tokens will be sent via postMessage to frontend (default)');

      // Optionally send tokens to backend if configured (legacy mode)
      if (this.config.backendUrl) {
        this.log('   Backend URL configured - will also support legacy direct-send mode');
        try {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      // Log all incoming requests (to file only in daemon mode)
      this.logToFileOnly(`📥 ${req.method} ${url.pathname}${url.search}`);
      ...
      if (url.pathname === '/' && !url.searchParams.has('code')) {
        ...
        if (preConfiguredTokens) {
          this.logToFileOnly(`🌐 Browser requested authentication (bypass mode)`);
          this.logToFileOnly(`   Return mode: ${returnTokens ? 'postMessage to frontend' : 'send to backend'}`);
          ...
          if (this.config.backendUrl) {
            try {
              ...
              this.logToFileOnly(`⚠️  Could not send tokens to backend: ${error.message}`);
            }
          }
        ...
        this.logToFileOnly(`🌐 Browser requested authentication, redirecting to OIDC provider`);
      ...
      if (url.pathname === '/' && url.searchParams.has('code')) {
        ...
        try {
          this.logToFileOnly('🔄 Exchanging authorization code for tokens...');
          ...
          this.logToFileOnly('✅ Tokens obtained successfully');
          this.logToFileOnly(`   Return mode: ${returnTokens ? 'postMessage to frontend' : 'send to backend'}`);
          ...
          if (this.config.backendUrl) {
            try {
              ...
              this.logToFileOnly(`✅ Tokens sent to Backstage backend`);
              ...
              try {
                ...
                backstageToken = responseData.backstageToken;
                if (backstageToken) {
                  this.logToFileOnly(`✅ Received Backstage session token`);
                }
              } catch (e) {
              ...
              this.logToFileOnly(`⚠️  Could not send tokens to Backstage: ${error.message}`);
              res.writeHead(200, { 'Content-Type': 'text/html' });
              res.end(`
                <html>
    ...
    return new Promise((resolve, reject) => {
      server.listen(this.config.callbackPort, '127.0.0.1', () => {
        ...
        this.log(`🔗 To authenticate, open: http://localhost:${this.config.callbackPort}/?mode=return-tokens`);
        this.log(`   Frontend will receive tokens via postMessage to window.opener`);
        if (this.config.backendUrl) {
          this.log(`📤 Backend URL (optional): ${this.config.backendUrl}`);
          this.log(`   Legacy mode: omit ?mode=return-tokens to send directly to backend`);
        }
        this.log('');
        resolve(server);
      });
  ...
  async sendTokensToBackend(tokens, backstageCookies = null) {
    ...
    // Filter out expires_in from the payload as it's inconsistent across OIDC providers
    // Backend should decode expiration from the JWT tokens themselves
    const { expires_in, ...tokenPayload } = tokens;

    this.logToFileOnly('📤 Sending tokens to backend...');
    this.logToFileOnly(`   Endpoint: ${this.config.backendUrl}/api/cluster-auth/tokens`);
    this.logToFileOnly(`   Payload:`);
    this.logToFileOnly(`     access_token: ${tokenPayload.access_token ? tokenPayload.access_token.substring(0, 30) + '...' : 'null'}`);
    this.logToFileOnly(`     id_token: ${tokenPayload.id_token ? tokenPayload.id_token.substring(0, 30) + '...' : 'null'}`);
    this.logToFileOnly(`     token_type: ${tokenPayload.token_type || 'Bearer'}`);
    if (expires_in) {
      this.logToFileOnly(`     expires_in: ${expires_in} (not sent to backend - decode from JWT instead)`);
    }
    if (tokenPayload.refresh_token) {
      this.logToFileOnly(`     refresh_token: ${tokenPayload.refresh_token.substring(0, 30)}...`);
    }
    if (backstageCookies) {
      this.logToFileOnly(`   ✅ Including Backstage authentication cookies`);
    } else {
      this.logToFileOnly(`   ⚠️  No Backstage cookies provided - backend requires authentication!`);
    }
    this.logToFileOnly('');
    ...
    return new Promise((resolve, reject) => {
      ...
      // Add auth secret if configured (required for backend authentication)
      if (this.config.backendSecret) {
        headers['X-Auth-Secret'] = this.config.backendSecret;
        this.logToFileOnly(`   🔐 Including auth secret in X-Auth-Secret header`);
      } else {
        this.logToFileOnly(`   ⚠️  No auth secret configured - backend may reject request!`);
      }

      // Forward Backstage session cookies if provided (legacy mode)
      if (backstageCookies) {
      ...
      const req = protocol.request(options, (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            this.logToFileOnly('✅ Tokens successfully sent to backend');
