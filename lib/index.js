...
class OIDCAuthenticator {
  ...
  formatTokenClaims(claims, indent = '     ') {
    ...
    // Standard claims to highlight
    const standardClaims = ['sub', 'iss', 'aud', 'exp', 'iat', 'nbf', 'email', 'name', 'preferred_username'];
    const otherClaims = [];

    // Separate standard and other claims
    for (const [key, value] of Object.entries(claims)) {
      if (standardClaims.includes(key)) {
        ...
        otherClaims.push([key, value]);
      }
    }

    // Add other claims
    if (otherClaims.length > 0) {
      lines.push(`${indent}... and ${otherClaims.length} other claims`);
    }
    ...
    const server = http.createServer(async (req, res) => {
      ...
      if (url.pathname === '/' && url.searchParams.has('code')) {
        ...
        try {
          ...
          // Send to Backstage backend if configured
          if (this.config.backendUrl) {
            try {
              await this.sendTokensToBackend(tokens);
              ...
              this.logError(`⚠️  Failed to send tokens to Backstage: ${error.message}`);
              res.writeHead(200, { 'Content-Type': 'text/html' });
              res.end(`
                <html>
                  <head><title>Authentication Partial Success</title></head>
                  <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; text-align: center;">
                    <h1 style="color: #ff9800;">⚠️ Authentication Completed</h1>
                    <p style="font-size: 18px; color: #666;">You authenticated successfully, but failed to send tokens to Backstage.</p>
  ...
  async authenticate() {
    ...
    if (tokens.access_token) {
      const decoded = this.decodeJWT(tokens.access_token);
      this.log('🔍 Access Token Claims:');
      if (decoded.type === 'jwt' && decoded.payload) {
        this.log(this.formatTokenClaims(decoded.payload));
      } else if (decoded.type === 'jwe') {
        this.log('     (JWE encrypted token - cannot decode without decryption key)');
      } else {
        this.log('     (Opaque token or unable to decode)');
      }
      this.log('');
    }
